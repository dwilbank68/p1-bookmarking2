...
class Nailer
  ...
  @@api_key = 'ea41497fa6cf37fac28365424c71a975'

  attr_accessor :collection_time, :job_id, :ok

  def initialize(url, width = 1024, height = 768)
    api_request = %Q{<webthumb><apikey>#{@@api_key}</apikey><request><url>#{url}</url><width>#{width}</width><height>#{height}</ght></request></webthumb>}
